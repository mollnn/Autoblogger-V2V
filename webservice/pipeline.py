...
def load(bvid):
    ...
    if len(common.query(common.readConfig("dbname"), "select * from state_board where bvid='%s' and `desc`='%s'" % (bvid, "media"))) == 0:
        ...
        common.query(common.readConfig(
            "dbname"), "insert ignore into state_board (bvid,`desc`) values ('%s','%s')" % (bvid, "media"))
    ...
    if len(common.query(common.readConfig("dbname"), "select * from state_board where bvid='%s' and `desc`='%s'" % (bvid, "info"))) == 0:
        ...
        common.query(common.readConfig(
            "dbname"), "insert ignore into state_board (bvid,`desc`) values ('%s','%s')" % (bvid, "info"))

    if len(common.query(common.readConfig("dbname"), "select * from state_board where bvid='%s' and `desc`='%s'" % (bvid, "cut"))) == 0:
        cutter.solve(bvid, 0)
        common.query(common.readConfig(
            "dbname"), "insert ignore into state_board (bvid,`desc`) values ('%s','%s')" % (bvid, "cut"))
...
def downloadTemplate(template_bvid):
    ...
    if len(common.query(common.readConfig("dbname"), "select * from state_board where bvid='%s' and `desc`='%s'" % (template_bvid, "media"))) == 0:
        ...
        common.query(common.readConfig(
            "dbname"), "insert ignore into state_board (bvid,`desc`) values ('%s','%s')" % (template_bvid, "media"))
...
def generate(src_type, clip_type):
    ...
    template_list = common.query(common.readConfig(
        "dbname"), "select bvid from in_templates where src_type=%d and clip_type=%d;" % (src_type, clip_type))
    handles = []
    ...
    def _solve_one(template_bvid):
        ...
        if len(common.query(common.readConfig("dbname"), "select * from state_gen where bvid='%s' and src_type=%d and clip_type=%d" % (template_bvid, src_type, clip_type))) == 0:
            ...
            common.query(common.readConfig("dbname"), "insert ignore into state_gen (ovid,`desc`,bvid,src_type,clip_type) values ('%s','%s','%s',%d,%d)" % (
                ovid, "ok", template_bvid, src_type, clip_type))
...
def edit():
    # 根据剪辑描述表剪辑视频
    print("edit")
    outvideo_list = common.query(common.readConfig(
        "dbname"), "select distinct ovid from out_editdesc;")
...
def main():
    ...
    common.query(common.readConfig("dbname_backend"),
                 """ truncate table state_exec; """)
