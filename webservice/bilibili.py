...
class Bilibili:
    ...
    def uploadvideo(self, configfile=None,bili_jct=None,cookievalue=None):
        cookie='SESSDATA=%s'%cookievalue
        uvideo=upload( bili_jct,cookie,configfile)
        for x in uvideo.upload():
            self._log(x)
        return True
...
def wrapper(arg):
    ...
    if instance.login(force_refresh_token=config['user']['force_refresh_token'], **account):
        threads = []
        if config['upload_video']['enable']:
            threads.append(threading.Thread(target=instance.uploadvideo,args=(config['upload_video']['videoconfig'],account['bili_jct'],account['SESSDATA'])))
